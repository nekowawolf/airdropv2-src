...
export default function EndedAirdropTable({ data }: EndedAirdropTableProps) {
  const [currentPage, setCurrentPage] = useState(1);

  const endedData = useMemo(() => {
    return data.filter(item => item.status === 'ended').reverse();
  }, [data]);

  const totalPages = Math.ceil(endedData.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const currentData = endedData.slice(startIndex, endIndex);

  useEffect(() => {
    setCurrentPage(1);
  }, [data]);

  const getVestingClass = (vesting: string | undefined) => {
    switch (vesting) {
      case 'yes':
        ...
        <table className="w-full text-sm text-left text-gray-500">
          <thead className="text-xs textp uppercase table-list transition-all duration-300">
            <tr>
              ...
              <th scope="col" className="py-3 px-6 text-center w-1/3">Vesting</th>
              <th scope="col" className="py-3 px-6 text-center w-1/3">Claim</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={airdrop.id} className="border-b border-underline">
                ...
                <td className="py-4 px-6 text-center align-middle">
                  <div className="flex items-center justify-center">
                    <div className={`flex items-center justify-center border rounded-md h-7 w-16 text-white ${getVestingClass(airdrop.vesting)}`}>
                      <span className="p-2">{airdrop.vesting?.toUpperCase() || 'N/A'}</span>
                    </div>
                  </div>
                </td>
                <td className="py-4 px-6 text-center">
